...
public final class AdminMonitorSettings {
    ...
    /** Ring supervisor desk phone via AMI (requires supervisor extension). */
    public static final String LISTEN_PHONE_AMI = "PHONE_AMI";
    /** Show dial code only (no AMI). */
    public static final String LISTEN_MANUAL = "MANUAL";

    private static final String FILE = "admin-monitor.properties";

    public String listenDelivery = LISTEN_PHONE_AMI;
    ...
    public static AdminMonitorSettings load() {
        ...
        s.listenDelivery = normalizeListenDelivery(p.getProperty("listenDelivery", LISTEN_PHONE_AMI));
    ...
    public void save() {
        Properties p = new Properties();
        p.setProperty("listenDelivery", listenDelivery == null ? LISTEN_PHONE_AMI : listenDelivery.trim());
    ...
    public boolean isListenConfigured() {
        if (LISTEN_MANUAL.equals(listenDelivery)) {
            return spyPrefix != null && !spyPrefix.isBlank();
        }
    ...
    private static String normalizeListenDelivery(String mode) {
        if (mode == null || mode.isBlank()) {
            return LISTEN_PHONE_AMI;
        }
        String m = mode.trim();
        if ("BUILTIN_WEBRTC".equals(m)) {
            return LISTEN_PHONE_AMI;
        }
        if (LISTEN_MANUAL.equals(m)) {
            return m;
        }
        return LISTEN_PHONE_AMI;
    }
